class Cube extends HTMLElement{
	...
	connectedCallback(){
		...
		this.createFace (vraiStyle, 'bottom', 'rotateX(-90deg) translateY(50%)', 'bottom center');
		if (this.getAttribute ('vertical') != null){
			this.sens = 'vertical';
			this.createFace (vraiStyle, 'back', 'rotateX(180deg) translateZ(' + height +'px)', null);
		}
		else this.createFace (vraiStyle, 'back', 'rotateY(180deg) translateZ(' + height +'px)', null);
		this.createFace (vraiStyle, 'left', 'rotateY(270deg) translateX(-50%)', 'center left');
		this.createFace (vraiStyle, 'right', 'rotateY(-270deg) translateX(50%)', 'center right');
		this.createFace (vraiStyle, 'front', 'translateZ(' + height +'px)', null);
		this.createFace (vraiStyle, 'top', 'rotateX(90deg) translateY(-50%)', 'top center');
...
class Rect extends Cube{
	connectedCallback(){
		...
		this.style.border = 'dotted 8px green';
	}
}
