...
function goStraight(){
	imageRotation.src = imageRef.dst;
	for (var screen of screens){
		if (screen.id === 'straight-screen') screen.className = 'current';
...
function validateStraight(){
	...
	document.getElementById ('straight-screen').className ="";
}
function rotateLeft(){
	imageRef.rotation -=2;
...
function rotateRight(){
	imageRef.rotation +=2;
	if (imageRef.rotation >90) imageRef.rotation = 90;
	imageRotation.style.transform = 'rotateZ(' + imageRef.rotation + 'deg)';
}