...
HTMLInputElement.prototype.printOne = function (varName, varValue){
	if (varValue.constructor.name == 'Array'){
		var nodeNew = null;
		var i=0;
		for (; i< varValue.length -1; i++){
			nodeNew = this.copy();
			nodeNew.value = varValue[i];
		}
		this.value = varValue[i];
	}
	else if (varValue.constructor.name == 'Object'){
		var nodeNew = null;
		for (var l in varValue) if (typeof (varValue[l]) != 'function'){
			nodeNew = this.copy();
			nodeNew.value = varValue[l];
		}
		this.remove();
	}
...
function dpInit(){
	...
	for (var v=1; v< bodyTmp.length; v++){
		...
		if (varListText.indexOf (varName) <0){
			varValue = getValueFromName (varName);
			if (varValue) document.body.printOne (varName, varValue);
			varListText.push (varName);
	}}
	// affichage des inputs
	var inputList = document.getElementsByTagName ('input');
	for (var v= inputList.length -1; v>=0; v--) if (exists (inputList[v].name)){
		varValue = getValueFromName (inputList[v].name);
		if (varValue) inputList[v].printOne (inputList[v].name, varValue);
	}
	...
	// liste
	for (var c=0; c< document.body.children.length; c++) document.body.children[c].listList();
}
