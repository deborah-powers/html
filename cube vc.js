...
class Crayon extends HTMLElement{
	constructor (nbFaces){
		super();
...
class Hexa extends Pole{
	...
	createShape(){
		...
		this.createBack();
		this.createBout();
		this.top.style.clipPath = 'polygon(25% 0, 75% 0, 100% 50%, 75% 100%, 25% 100%, 0 50%)';
		this.bottom.style.clipPath = 'polygon(25% 0, 75% 0, 100% 50%, 75% 100%, 25% 100%, 0 50%)';
		this.left = this.createFace (this.children[3], 'left', 'rotateY(-60deg) translateZ(' + this.depth + 'px)');
		this.right = this.createFace (this.children[4], 'right', 'rotateY(60deg) translateZ(' + this.depth + 'px)');
		this.createFront();
		this.leftBack = this.createFace (this.children[6], 'left-back', 'rotateY(-120deg) translateZ(' + this.depth + 'px)');
		this.rightBack = this.createFace (this.children[7], 'right-back', 'rotateY(120deg) translateZ(' + this.depth + 'px)');
		this.back.style.left = '25%';
		this.back.style.width = '50%';
		this.front.style.left = '25%';
		this.front.style.width = '50%';
		this.left.style.left = '25%';
		this.left.style.width = '50%';
		this.right.style.left = '25%';
		this.right.style.width = '50%';
		this.leftBack.style.left = '25%';
		this.leftBack.style.width = '50%';
		this.rightBack.style.left = '25%';
		this.rightBack.style.width = '50%';
	}
	createBout(){
		this.top.style.clipPath = 'polygon(25% 0, 75% 0, 100% 50%, 75% 100%, 25% 100%, 0 50%)';
		this.bottom.style.clipPath = 'polygon(25% 0, 75% 0, 100% 50%, 75% 100%, 25% 100%, 0 50%)';
...
class Octo extends Pole{
	...
	createFacade (face, name, transform, width){
		super.createFace (face, name, transform)
}}
/*
